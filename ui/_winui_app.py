...
This module contains the actual WinUI implementation. Importing it will import
`win32more` WinUI bindings, which triggers Windows App SDK initialization and
therefore requires the Windows App Runtime to be installed.

Use `ui.app` for an import-safe wrapper (useful for CI / headless environments).
"""
...
class SafeSalesWinUIApp(XamlApplication):
    ...
    def _on_process_excel(self, sender, args):
        ...
        def worker():
            ...
            lines = "\n".join(result.lines)
            self._enqueue(
                lambda ft=ft, st=st, ship=result.shipments, ln=lines: self._finish_process(
                    ship,
                    ft,
                    st,
                    ln,
